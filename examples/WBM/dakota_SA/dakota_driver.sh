...
cd ../
cp mosdak_2D.template workdir.$num/
#cp mesh_file.e workdir.$num/
cp dprepro workdir.$num/
cp mosdak_2D.i workdir.$num/

# RUN the simulation from workdir.num
cd workdir.$num/
#input params.in into the moose simulation
./dprepro --left-delimiter=% --right-delimiter=% params.in mosdak_2D.template mosdak_2D.i

# --------
# ANALYSIS
# --------
#
mpirun -np 8 ../../picachu-opt -i mosdak_2D.i > temp.out
...
tail -1 mosdak_2D_out.csv|cut -f1 -d',' >> results.tmp
tail -1 mosdak_2D_out.csv|cut -f2 -d',' >> results.tmp
