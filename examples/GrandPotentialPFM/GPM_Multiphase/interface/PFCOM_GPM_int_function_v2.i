...
[Functions]
  [./ic_func_etaa0]
    type = ParsedFunction
    value = 'int_thick:=0.05; 0.5^2*(1.0-tanh(pi*(x-1.0)/int_thick))*(1.0+tanh(pi*(-y+10.0)/int_thick))'
  [../]
  [./ic_func_etab0]
    type = ParsedFunction
    value = 'int_thick:=0.05; 0.5^2*(1.0+tanh(pi*(x-1.0)/int_thick))*(1.0+tanh(pi*(-y+10.0)/int_thick))'
  ...
  [./ic_func_etag0]
    type = ParsedFunction
    value = 'int_thick:=0.05; 0.5*(1.0+tanh(pi*(y-10.0)/int_thick))'
  [../]
  [./ic_func_oxygen]
    type = ParsedFunction
    value = 'int_thick:=0.05; -3*(0.5^2*(1.0+tanh(pi*(x-1.0)/int_thick))*(1.0+tanh(pi*(-y+10.0)/int_thick)))'
  [../]
[]
...
[Materials]
  ...
  [./phase_mobility]
    ...
    constant_expressions  = '0.1      10       10       0.1       100     200'
  ...
  [./reaction_rates]
    ...
    constant_expressions  = '0         -10          -10         0         100     200'
  ...
  # Constant parameters
  [./constants]
    type = GenericConstantMaterial #0.1 constants worked
    prop_names =  'kappa  Va    L_a  L_b  L_g'
    prop_values = '0.001   1.0  0.1  0.2  1.0'
    outputs = exodus
  [../]
  ...
  [./diff_o]
    ...
    args = 'etaa0 etab0 etag0'
    material_property_names = 'h_a h_b h_g'
    function = '(h_a*1e-10 + h_b*0.1 +h_g*1)'
